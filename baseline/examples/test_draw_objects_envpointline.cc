 ...
 * @Date: 2023-08-27 09:46:30
 * @LastEditTime: 2023-08-27 15:28:23
 * @LastEditors: yanyan-li yanyan.li.camp@gmail.com
 * @Description:
 * @FilePath: /JointFactorGraph/examples/test_draw_objects_envpointline.cc
 */
...
void CyclePoseGeneration(double theta,
                         double wave_theta,
                         double step,
                         int radius,
                         double wave_high,
...
bool GenerateCycleKeyFrames(const int frame_num,
                            int wave_num,
                            double wave_high,
                            double radius_cycle,
                            std::vector<Eigen::Matrix4d> &vec_traject_gt_Twc)
{
    ...
    for (int n = 0; n < frame_num; n++) // camera id from n=0 to n=frameNum-1
    {
        ...
        CyclePoseGeneration(theta, wave_theta, step, radius, wave_high, n, vec_traject_gt_Twc);
    }
}
...
void GetBoundsOfPlane(std::vector<Eigen::Vector3d> &vertices, Eigen::Vector3d &max_bounds, Eigen::Vector3d &min_bounds)
{
    ...
    Eigen::Vector3d a_axis = (vertices[0] - vertices[1]).normalized();
    Eigen::Vector3d b_axis = (vertices[0] - vertices[2]).normalized();

    Eigen::Vector3d normal = (a_axis.cross(b_axis)).normalized();
...
void BuildObjectScene(std::map<std::string, std::vector<Eigen::Vector3d>> &cube_top_planes,
                      ...
                      std::map<std::string, std::vector<Eigen::Vector3d>> &cube_5_planes,
...
int main(/*int argc, char* argv[]*/)
{
    ...
    std::map<std::string, std::vector<Eigen::Vector3d>> floor_planes;
    ...
    objects_env.GetRegions(floor_planes, box_1_planes, box_2_planes, box_3_planes, box_4_planes);

    std::vector<Eigen::Vector3d> env_mappoints;
    std::vector<Eigen::Matrix<double, 3, 2>> env_maplines;
    BuildObjectScene(floor_planes,
                     box_1_planes,
                     box_2_planes,
                     box_3_planes,
                     box_4_planes,
    ...
    GenerateCycleKeyFrames(100, wave_num, wave_high, 5, vec_traject_gt_Twc);

    Eigen::Vector3d color(0.1, 0.5, 0.1);
    Eigen::Vector3d frame_line_color(0.5, 0.1, 0.1);
    Eigen::Vector3d frame_line_color_1(0.2, 0.1, 0.2);
    ...
    while (!pangolin::ShouldQuit())
    {
        ...
        if (d_cam.IsShown())
        {
            ...
            AddScenePart(floor_planes, frame_line_color);
            AddScenePart(box_1_planes, frame_line_color_2);
            AddScenePart(box_2_planes, frame_line_color_1);
            AddScenePart(box_3_planes, frame_line_color_4);
            AddScenePart(box_4_planes, frame_line_color_3);
            DrawOrigin();
            DrawCoordiante();
