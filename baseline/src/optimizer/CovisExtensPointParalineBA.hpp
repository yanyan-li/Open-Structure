 ...
 * @Date: 2023-03-07 14:34:50
 * @LastEditTime: 2023-08-28 10:18:35
 * @LastEditors: yanyan-li yanyan.li.camp@gmail.com
 * @Description:
 * @FilePath: /JointFactorGraph/src/optimizer/CovisExtensPointParalineBA.hpp
 */
#ifndef __VENOM_SRC_OPTIMIZER_COVISEXTE_EXPLICITE_GRAPH_BUNDLEADJUSTMENT__
#define __VENOM_SRC_OPTIMIZER_COVISEXTE_EXPLICITE_GRAPH_BUNDLEADJUSTMENT__
...
namespace simulator
{
    namespace optimizer
    {
        class CovisExtensPointParalineBA
        {
        public:
                ...
                std::map<int, Eigen::Matrix4d> &optimized_poses)
            {
                ...
                for (auto mp = opti_para.mappoints.begin(); mp != opti_para.mappoints.end(); mp++)
                {
                    ...
                    if (opti_para.mappoints[mp_id] == Vec3::Zero())
                        continue;
                    if (opti_para.mappoints[mp_id].norm() < 0.0001)
                        continue;
                    // TODO: it means the mp_id is less than mappoint.size
                ...
                for (auto &parali_id : opti_para.asso_paralineid_mlids)
                {
                    ...
                    std::cout << "***** a new para***" << std::endl;
                    parallel_candidate(parali_id.second, opti_para.maplines, para_inlier, individual_line);
                    ...
                    for (int i = 0; i < individual_line.size(); i++)
                    {
                        std::vector<int> single_line;
                        single_line.push_back(individual_line[i]);
                        paralineid_mlids[vanishing_id] = single_line;
                        vanishing_id += 1;
                    }
                }
                ...
                for (auto &parali_id : paralineid_mlids)
                {
                    ...
                    for (auto &ml_id : parali_id.second)
                    {
                        ...
                        if (opti_para.maplines[ml_id].col(0).norm() < 0.001 && opti_para.maplines[ml_id].col(1).norm() < 0.001)
                            continue;
                ...
                for (auto ml = opti_para.maplines.begin(); ml != opti_para.maplines.end(); ml++)
                {
                    ...
                    std::cout << "check : " << StartPoint << ", " << EndPoint << std::endl;
                    Eigen::Vector3d LineDirection = EndPoint - StartPoint;
                    Eigen::Vector3d LinePlaneNormal = StartPoint.cross(EndPoint);
                    double d = LinePlaneNormal.norm() / LineDirection.norm();
                    LinePlaneNormal = LinePlaneNormal.normalized();
                    ...
                    for (int i = 0; i < asso_mpl_mea->second.size(); i++)
                    {
                        ...
                        for (auto &ml_meas : opti_para.asso_ml_meas[ml_id])
                        {
                            ...
                            problem.AddResidualBlock(rot_cost_function, loss_function,
                                                     Para_Pose[frame_id_i], paraline_orth_shared[mpl_id].data());
                        }
                ...
                for (int inter = 0; inter < 100; inter++)
                {
                    ...
                    for (auto &ml_id : mpli_ids)
                    {
                        ...
                        if (ml_id == mpli_ids[random_number])
                            continue;
                        if (line_direction.transpose() * line_direction_seed > 0.84)
                            inliers.push_back(ml_id);
                        else if (-line_direction.transpose() * line_direction_seed > 0.84)
                        {
                            inliers.push_back(ml_id);
                            // exchange
                            Eigen::Vector3d startpoint = mplis[ml_id].col(0);
...
#endif // __VENOM_SRC_OPTIMIZER_COVISEXTE_EXPLICITE_GRAPH_BUNDLEADJUSTMENT__
